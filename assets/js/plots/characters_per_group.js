...
var svg12 = d3.select("#characters_per_group")
  .append("svg")
...
d3.csv("data/balanced_df.csv", function(data) {
  ...
  var sumstat = d3.nest() // nest function allows to group the calculation per level of a factor
    ...
    .rollup(function(d) {
      q1 = d3.quantile(d.map(function(g) { return g.number_of_characters;}).sort(d3.ascending), .25);
      median = d3.quantile(d.map(function(g) { return g.number_of_characters;}).sort(d3.ascending), .5);
      q3 = d3.quantile(d.map(function(g) { return g.number_of_characters;}).sort(d3.ascending), .75);
      interQuantileRange = q3 - q1;
      min = Math.max(1, q1 - 1.5 * interQuantileRange);
  ...
  var y = d3.scaleLinear()
    .domain([0,10])
    .range([height, 0])
  svg12.append("g").call(d3.axisLeft(y))
  ...
  svg12.append("text")
    ...
    .text("Normalized Number of Characters per Treatment Group");
  ...
  svg12.append("text")
    ...
    .text("Normalized Number of Characters");
